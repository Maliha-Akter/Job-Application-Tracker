...
we create a new element using `document.createElement()` and insert it using methods like `appendChild()` into parent.
Example:
...
**Event Bubbling** is when something we do on a child element (like clicking a button) ,the click also effect on its parent elements automatically, one by one, going up the DOM tree until the end of tree(document).
...
***
It means: child -> parent -> parent of  parents ->....-> document.  
...
**`preventDefault()`**  
  ...
  --> Example:It prevent a link from opening a page when clicked.
